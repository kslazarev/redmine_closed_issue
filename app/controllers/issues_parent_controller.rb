...
class IssuesParentController < ApplicationController
  unloadable
  menu_item :issues_parent, :only => [:index]
  ...
  before_filter :authorize, :except => [:index, :statistics]
  before_filter :find_optional_project, :only => [:index, :statistics]
  before_filter :check_for_default_issue_status, :only => [:new, :create]
  before_filter :build_new_issue_from_params, :only => [:new, :create]
  accept_rss_auth :index, :show, :statistics
  accept_api_auth :index, :show, :create, :update, :destroy, :statistics

  rescue_from Query::StatementInvalid, :with => :query_statement_invalid

  include IssuesParentHelper
  helper :journals
  helper :projects
  ...
  helper :queries
  include CustomParentQueriesHelper
  helper :repositories
  include RepositoriesHelper
  helper :sort
  include SortHelper
  include IssuesStatisticsHelper
  helper :timelog
  helper :gantt
  ...
  def index
    ...
    if @query.valid?
      ...
      respond_to do |format|
        format.html { render :template => 'issues_parent/index', :layout => !request.xhr? }
        format.api {
  ...
  def find_issue
    ...
    @issue = Issue.find(params[:id], :include => [:project, :tracker])
